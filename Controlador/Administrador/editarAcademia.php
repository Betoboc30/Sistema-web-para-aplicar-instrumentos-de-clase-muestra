...
if (
    isset($_POST['idacademia']) && isset($_POST['academia'])
    && isset($_POST['periodo'])
) {

    //Obtenemos los valores de formulario y los asignamos a variables
    $idAcademia = htmlspecialchars($_POST['idacademia'], ENT_QUOTES, 'UTF-8');
    $academia = htmlspecialchars($_POST['academia'], ENT_QUOTES, 'UTF-8');
    $periodo = htmlspecialchars($_POST['periodo'], ENT_QUOTES, 'UTF-8');


    $sql = $conexion->query("UPDATE academia SET NombreAca='$academia', Periodo=$periodo WHERE idAcademia=$idAcademia");
    ...
    if ($sql) {
        ...
        header("Refresh:3; ../../Vista/Administrador/gestionAcademia.php");
    } else {
